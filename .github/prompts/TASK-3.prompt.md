---
mode: 'agent'
description: 'TASK-3: Audit Missing Docs and Create Per-Section Content Tasks'
---
...
## Steps
1. [ ] **Audit all 27 empty MDX files** — for each file, note: intended audience, what Syntra feature it covers, and which source files in `ai/` (agents, instructions, config) or the project README apply.
...
2. [ ] **Cross-reference source material** — for each section, map relevant source files:
...
3. [ ] **Create TASK-4** — `getting-started` section content (index overview, concepts clarification if needed).

4. [ ] **Create TASK-5** — `commands` section content (all CLI commands: init, add, sync, done, validate, template).

5. [ ] **Create TASK-6** — `configuration` section content (config reference, templates, project detection).

6. [ ] **Create TASK-7** — `guides` section content (first project, writing tasks, team workflow, migrating).

7. [ ] **Create TASK-8** — `agents` section content (index, code-generator, bug-fixer, code-reviewer, task-planner, custom).

8. [ ] **Create TASK-9** — `adapters` section content (index, claude-code, copilot, cursor, custom).

9. [ ] **Create TASK-10** — `instructions` section content (index, architecture, deploy, security, style, testing, custom).

10. [ ] **Run `npm run build`** — confirm all pages compile and no broken static paths or MDX parsing errors.

## Acceptance Criteria
- [ ] All 27 empty MDX files are catalogued with a clear content brief (what to write, what source to draw from).
- [ ] 7 sub-tasks (TASK-4 through TASK-10) are created in `ai/tasks/active/`, each scoped to one section.
- [ ] Each sub-task references the relevant source files and has specific acceptance criteria.
- [ ] `npm run build` passes with no errors before this task is marked done.
